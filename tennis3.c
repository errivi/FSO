...
void * moure_pilota(void * cap)
{
  ...
  do{
	...
	pthread_mutex_unlock(&mutex_pausa); 
  	...
  	win_retard(retard);
  }while((tec != TEC_RETURN) && (cont==-1) && ((*p_moviments > 0) /*|| *p_moviments == -1)*/ || mov_desactivats==1));
  i=1;
...
int main(int n_args, const char *ll_args[])
{
  ...
  int id_final;
  char a1[20], a2[20], a3[20], a4[20], a5[20], a6[20], a7[20], a8[20], /*mapa[20],*/ fila[20], col[20], mov[20];
  ...
  *p_moviments = moviments;
  ...
  for (int i=0; i<pal_ord_actuals; i++){
  	...
  	if(tpid[i] == (pid_t)0){
  		sprintf(a7, "%i", i);
  		execlp("./pal_ord3", "pal_ord3", a1, a2, a3, a4, a5, a6, a7, a8, mapa, fila, col, mov, (char *)0);
  ...
  *p_final=true;
  ...
  else { if (cont == 0 || *p_moviments == 0) printf("Ha guanyat l'ordinador!\n");
