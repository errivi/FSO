...
int moviments, mov_actuals, mov_totals;		/* numero max de moviments paletes per acabar el joc */
int pal_ord_actuals=0;

int tec, cont;

bool mov_desactivats, pausa;
	...
	pthread_mutex_unlock(&mutex_pausa); 
	  ...
	  {		/* si posicio hipotetica no coincideix amb la pos. actual */
		...
		{    pil_vf = -pil_vf; pil_vc = -pil_vc;	/* canvia velocitats */
    		...
    		else pthread_mutex_unlock(&mutex_pant);
    		
  	  }
  	else { pil_pf += pil_vf; pil_pc += pil_vc; }
  	win_retard(retard);
  }while((tec != TEC_RETURN) && (cont==-1) && (((moviments > 0) || moviments == -1) || mov_desactivats));
  i=1;
...
void * mou_paleta_ordinador(void * index)
{
  ...
  do{
  	...
  	pthread_mutex_lock(&mutex_pausa); //bloqueig del fil si es prem espai
	pthread_mutex_unlock(&mutex_pausa);
  	...
  	if (f_h != ipo_pf[ind])	/* si pos. hipotetica no coincideix amb pos. actual */
  	{
			...
			if (win_quincar(f_h+l_pal-1,ipo_pc[ind]) == ' ')   /* si no hi ha obstacle */
			{
	  			...
	  			pthread_mutex_lock(&mutex_pant);
	  			win_escricar(ipo_pf[ind]+l_pal-1,ipo_pc[ind],'1',INVERS); /* impr. ultim bloc */
	  			pthread_mutex_unlock(&mutex_pant);
          			if (moviments > 0) {
          				pthread_mutex_lock(&mutex_var);
          				moviments--;    /* he fet un moviment de la paleta */
			...
			if (win_quincar(f_h,ipo_pc[ind]) == ' ')        /* si no hi ha obstacle */
			{
	  			...
	  			po_pf[ind] += v_pal[ind]; ipo_pf[ind] = po_pf[ind];		/* actualitza posicio */
	  			pthread_mutex_lock(&mutex_pant);
	  			win_escricar(ipo_pf[ind],ipo_pc[ind],'1',INVERS);	/* impr. primer bloc */
	  			pthread_mutex_unlock(&mutex_pant);
          			if (moviments > 0) {
          				pthread_mutex_lock(&mutex_var);
          				moviments--;    /* he fet un moviment de la paleta */
      ...
      else po_pf[ind] += v_pal[ind]; 	/* actualitza posicio vertical real de la paleta */
...
int main(int n_args, const char *ll_args[])
{
  ...
  if(moviments==0) mov_desactivats=true;
  else mov_desactivats=false;
  ...
  for (int i=0; i<pal_ord_actuals; i++){
  	...
  	if(pthread_create(&tid[n],NULL,mou_paleta_ordinador, (void *) (intptr_t) i)==0){
  		n++;
  	}
  }
  ...
  do{				/********** bucle tasca independent **********/	
  	...
  	mov_actuals = mov_totals-moviments;
