...
void * mou_paleta_ordinador(void * index)
{
  ...
  do{
  	f_h = po_pf[(intptr_t)index] + v_pal[(intptr_t)index];		/* posicio hipotetica de la paleta */
  	if (f_h != ipo_pf[(intptr_t)index])	/* si pos. hipotetica no coincideix amb pos. actual */
  	{
    		if (v_pal[(intptr_t)index] > 0.0)			/* verificar moviment cap avall */
    		{
			if (win_quincar(f_h+l_pal-1,ipo_pc[(intptr_t)index]) == ' ')   /* si no hi ha obstacle */
			{
	  			win_escricar(ipo_pf[(intptr_t)index],ipo_pc[(intptr_t)index],' ',NO_INV);      /* esborra primer bloc */
	  			po_pf[(intptr_t)index] += v_pal[(intptr_t)index]; ipo_pf[(intptr_t)index] = po_pf[(intptr_t)index];		/* actualitza posicio */
	  			win_escricar(ipo_pf[(intptr_t)index]+l_pal-1,ipo_pc[(intptr_t)index],'1',INVERS); /* impr. ultim bloc */
          			if (moviments > 0) moviments--;    /* he fet un moviment de la paleta */
			}
			else		/* si hi ha obstacle, canvia el sentit del moviment */
	   			v_pal[(intptr_t)index] = -v_pal[(intptr_t)index];
       		}
       		else			/* verificar moviment cap amunt */
       		{
		if (win_quincar(f_h,ipo_pc[(intptr_t)index]) == ' ')        /* si no hi ha obstacle */
		{
	  	win_escricar(ipo_pf[(intptr_t)index]+l_pal-1,ipo_pc[(intptr_t)index],' ',NO_INV); /* esbo. ultim bloc */
	  	po_pf[(intptr_t)index] += v_pal[(intptr_t)index]; ipo_pf[(intptr_t)index] = po_pf[(intptr_t)index];		/* actualitza posicio */
	  	win_escricar(ipo_pf[(intptr_t)index],ipo_pc[(intptr_t)index],'1',INVERS);	/* impr. primer bloc */
          	if (moviments > 0) moviments--;    /* he fet un moviment de la paleta */
		}
		else		/* si hi ha obstacle, canvia el sentit del moviment */
	   		v_pal[(intptr_t)index] = -v_pal[(intptr_t)index];
      ...
      else po_pf[(intptr_t)index] += v_pal[(intptr_t)index]; 	/* actualitza posicio vertical real de la paleta */
      win_retard(retard);
 }while((tec != TEC_RETURN) && (cont==-1) && ((moviments > 0) || moviments == -1));
  i=1;
  return((void *) (intptr_t) i);
...
int main(int n_args, const char *ll_args[])
{
  ...
  for (int i=0; i<pal_ord_actuals; i++){
  	if(pthread_create(&tid[2],NULL,mou_paleta_ordinador, (void *) (intptr_t) i)==0){
  		n++;
  	}
  }
  printf("S'ha creat %d fils en total\n", n);
  int segons=0;
  int minuts=0;
  char string[50];
  do{				/********** bucle tasca independent **********/	
  	sprintf(string, "Temps de joc: %d:%d ", minuts, segons);
  	win_escristr(string);	
  	segons++;
  	if (segons==60){
  		segons=0;
